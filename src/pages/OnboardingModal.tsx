...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 dark:bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#111827] rounded-2xl max-w-xl w-full shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {step === 'intro' && (
          <div className="p-6 space-y-5">
            <div className="text-center py-2">
              <div className="w-16 h-16 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/50 rounded-2xl mx-auto flex items-center justify-center text-blue-700 dark:text-blue-300 mb-3 shadow-inner">
                <FileCheck className="w-8 h-8" />
              </div>
              <h4 className="text-lg font-bold text-slate-900 dark:text-white">Connect your digital documents</h4>
            ...
            <div className="bg-slate-50 dark:bg-[#162033] border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-2.5 text-xs text-slate-700 dark:text-slate-300">
              <div className="font-semibold text-slate-900 dark:text-white flex items-center gap-1.5">
                <Lock className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                <span>Simulated DigiLocker Credentials Available:</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div className="p-2 bg-white dark:bg-[#111827] rounded-lg border border-slate-200 dark:border-slate-700 flex items-center gap-2 text-slate-700 dark:text-slate-300">
        ...
        {step === 'consent' && (
          <div className="p-6 space-y-5">
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-slate-50 dark:bg-[#162033] border border-slate-200 dark:border-slate-800 rounded-xl space-y-1.5">
                <span className="font-bold text-slate-900 dark:text-white">1. Information Requested:</span>
                <div className="flex flex-wrap gap-1.5 pt-1">
                  <span className="px-2 py-0.5 bg-white dark:bg-[#111827] border border-slate-300 dark:border-slate-700 rounded text-slate-700 dark:text-slate-300 font-medium">
              ...
              <div className="p-3 bg-slate-50 dark:bg-[#162033] border border-slate-200 dark:border-slate-800 rounded-xl">
                <span className="font-bold text-slate-900 dark:text-white">2. Purpose of Access:</span>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-slate-50 dark:bg-[#162033] border border-slate-200 dark:border-slate-800 rounded-xl">
                  <span className="font-bold text-slate-900 dark:text-white">3. Access Duration:</span>
                  <p className="text-slate-600 dark:text-slate-300 mt-1">Valid for current session or until explicit revocation.</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-[#162033] border border-slate-200 dark:border-slate-800 rounded-xl">
                  <span className="font-bold text-slate-900 dark:text-white">4. Revocation Policy:</span>
                  <p className="text-slate-600 dark:text-slate-300 mt-1">You can revoke access anytime from the Data &amp; Consent center.</p>
                </div>
              </div>
            </div>

            {/* Warning / Non-Generic notice */}
            <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 text-xs text-amber-900 dark:text-amber-300 flex items-start gap-2">
        ...
        {step === 'connected' && (
          <div className="p-6 space-y-5 text-center">
            <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/50 rounded-2xl mx-auto flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-inner">
              <CheckCircle2 className="w-8 h-8" />
            </div>
            <div>
              <span className="inline-block px-2.5 py-0.5 text-xs font-bold bg-emerald-100 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800/50 rounded-full mb-1">
                Connected &amp; Verified
              </span>
              <h4 className="text-lg font-bold text-slate-900 dark:text-white">DigiLocker Mock Adapter Linked</h4>
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 max-w-md mx-auto">
                Your verified digital identity and educational records are ready for one-click application pre-filling.
              </p>
            </div>

            <div className="p-4 bg-slate-50 dark:bg-[#162033] border border-slate-200 dark:border-slate-800 rounded-xl text-left text-xs space-y-2">
              ...
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                ...
                <span className="font-mono text-slate-700 dark:text-slate-300">{citizen.connectedAt || '01 Sep 2026'}</span>
              </div>
              <div className="flex justify-between text-slate-600 dark:text-slate-400">
                <span>Adapter Type:</span>
                <span className="font-semibold text-amber-700 dark:text-amber-400">Simulated DigiLocker Adapter</span>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-2.5 border border-rose-200 dark:border-rose-900/50 text-rose-700 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl text-xs font-semibold transition-colors"
              >
                Disconnect Adapter
              </button>
              <button
                onClick={onClose}
                className="flex-1 py-2.5 bg-navy-900 dark:bg-brand-600 hover:bg-navy-800 dark:hover:bg-brand-700 text-white rounded-xl text-xs font-semibold shadow-md transition-colors"
              >
                Done
              </button>
            </div>
