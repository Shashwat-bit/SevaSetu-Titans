...
export const OfficerDashboardPage: React.FC<OfficerDashboardPageProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-gradient-to-r from-navy-900 via-navy-800 to-slate-900 rounded-2xl p-6 sm:p-8 text-white shadow-lg border border-navy-700/60 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-navy-800/80 border border-navy-600 text-xs text-amber-300 font-semibold shadow-inner">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-2 px-3.5 py-2 text-xs font-semibold bg-navy-800/90 hover:bg-navy-700 text-slate-200 rounded-xl border border-navy-600 transition-all shadow-sm"
            >
        ...
        <div className="mt-6 pt-4 border-t border-navy-700/60 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-400">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-emerald-400" />
            <span>Dispatched via SevaSetu Interoperability Layer to Mock Department Adapter</span>
          </div>
          <span className="text-[11px] px-2 py-0.5 rounded bg-navy-800 text-amber-300 font-mono">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        >
          ...
          <span className="text-[11px] text-slate-500 font-medium">Department Applications</span>
        </div>

        {/* Pending Verification */}
        <div
          onClick={() => onNavigateToApplications('Under Verification')}
          className="bg-white rounded-xl border border-amber-200/80 p-4 shadow-sm hover:shadow-md hover:border-amber-400 transition-all cursor-pointer group bg-gradient-to-b from-amber-50/20 to-white"
        >
          ...
          <span className="text-[11px] text-amber-700 font-medium">Docs Require Sign-off</span>
        </div>

        {/* Under Review */}
        <div
          onClick={() => onNavigateToApplications('Under Review')}
          className="bg-white rounded-xl border border-sky-200/80 p-4 shadow-sm hover:shadow-md hover:border-sky-400 transition-all cursor-pointer group bg-gradient-to-b from-sky-50/20 to-white"
        >
          ...
          <span className="text-[11px] text-sky-700 font-medium">Desk Scrutiny Stage</span>
        </div>

        {/* Approved */}
        <div
          onClick={() => onNavigateToApplications('Approved')}
          className="bg-white rounded-xl border border-emerald-200/80 p-4 shadow-sm hover:shadow-md hover:border-emerald-400 transition-all cursor-pointer group bg-gradient-to-b from-emerald-50/20 to-white"
        >
          ...
          <span className="text-[11px] text-emerald-700 font-medium">Sanctioned & Completed</span>
        </div>

        {/* Rejected */}
        <div
          onClick={() => onNavigateToApplications('Rejected')}
          className="bg-white rounded-xl border border-rose-200/80 p-4 shadow-sm hover:shadow-md hover:border-rose-400 transition-all cursor-pointer group bg-gradient-to-b from-rose-50/20 to-white"
        >
          ...
          <span className="text-[11px] text-rose-700 font-medium">Ineligible or Flagged</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-5 sm:p-6 border-b border-slate-100 flex items-center justify-between gap-4">
          <div>
            <h2 className="text-base font-bold text-navy-900">Recent Desk Applications</h2>
                  ...
                  <tr key={app.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-navy-50 text-navy-800 hover:bg-navy-100 font-semibold border border-navy-200 transition-colors shadow-xs"
                      >
                        Review
                        <ArrowRight className="w-3.5 h-3.5" />
                      </button>
